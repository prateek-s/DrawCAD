...
public class DrawCADCircleFitter
{
	...
	public void doFit(double[][] stroke) throws Exception
	{
		...
		noOfPoints = noOfErrorEquations = stroke.length;
		errors=new Matrix(stroke.length , 1);//new double[points.length-1];
	...
	public void newton()
	{
		...
		for (k = 0; k<iterationsLimit ; k++)
		{
			for(i=0;i<noOfErrorEquations;i++)
			{
				x1=points.get(i,0);
				y1=points.get(i,1);
				double temp =  (centerX-x1)*(centerX-x1) + (centerY-y1)*(centerY-y1) - radius*radius ;
				errors.set(i,0, -1 * temp );
				Jacobian.set( i , 0 , 2*(centerX-x1) );
				Jacobian.set( i , 1 , 2*(centerY-y1) );
				Jacobian.set( i , 2 , -2*radius );
/*				errors.set(i,0, -1 * (temp * temp ) );
				Jacobian.set( i , 0 , 2*temp * 2*(centerX-x1) );
				Jacobian.set( i , 1 , 2*temp * 2*(centerY-y1) );
				Jacobian.set( i , 2 , (-1) * 2*temp * 2*radius );*/
			}
