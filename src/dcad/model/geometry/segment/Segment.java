...
public abstract class Segment extends GeometryElement
{
	...
	*/
	...
	/**
	 * what is the catagory of this segment as detected by the recognition algo
	 */ 
...
//		setM_points(points);
	...
	public void changeAnchorPoint(AnchorPoint ap1, AnchorPoint ap2)
	{
		for(int l=0;l<m_impPoints.size();l++)
			if(m_impPoints.elementAt(l)==ap1)
			{
				m_impPoints.remove(ap1);
				m_impPoints.add(l,ap2);
			}
	}
...
//	public abstract void movePt();
	...
	public Vector getConstraintWithSeg(Segment seg, Class classname)
	{
		...
		if(closeSegments!=null)
		{
			...
			while (iter.hasNext())
			{
				...
				if(!seg.equals(this))
				{
					...
					Vector constraints = getConstraintWithSeg(seg, IntersectionConstraint.class);
					constraints.addAll(getConstraintWithSeg(seg, lineCircularCurveTangencyConstraint.class));
					constraints.addAll(getConstraintWithSeg(seg, twoCircularCurveTangencyConstraint.class));
