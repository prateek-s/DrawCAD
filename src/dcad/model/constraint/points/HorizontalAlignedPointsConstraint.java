...
public class HorizontalAlignedPointsConstraint extends IndependentPointConstraints
{
	public HorizontalAlignedPointsConstraint(AnchorPoint ap1, AnchorPoint ap2,int category,boolean promoted)
	{
	...
	public String toString()
	{
		...
		return addPrefix()+" Horizontally aligned Points : " + m_ap1.getM_strId() + " and " + m_ap2.getM_strId();
	}
	...
	public Vector getEquation(Vector fixedPoints)
	{
		...
		//retVec.add(constraintEquations.getHorizontalLineSegmentEQ(pointStrings[0],pointStrings[1]));
		retVec.add( constraintsHelper.horizontalConstraintScaleFactor + "*"+constraintEquations.getYDifferenceEQ(pointStrings[0],pointStrings[1]) );
		return retVec;
	}
	...
	public Vector getPartialDifferentialString(String var, Vector fixedPoints)
	{
		...
		AnchorPoint[] anchorPoints=constraintsHelper.getAnchorPoints(points);
		String[][] pointStrings=constraintsHelper.getpointStrings(anchorPoints);
		//retVec.add(constraintEquations.getHorizontalLineSegmentPD(pt, xory, anchorPoints[0],anchorPoints[1],pointStrings[0],pointStrings[1]));
		retVec.add(constraintsHelper.horizontalConstraintScaleFactor + "*"+constraintEquations.getYDifferencePD(pt,xory,anchorPoints[0],anchorPoints[1],pointStrings[0],pointStrings[1]));
		return retVec;
	}
	...
	public boolean isConstraintSolved()
	{
		...
		if(Math.abs(  ((AnchorPoint)points.get(0)).getY() -  ((AnchorPoint)points.get(1)).getY()  )<constraintsHelper.pointOverlapErrorThreshold)
			return true;
		constraintsHelper.printConstraintSolvingFailure("HorizontalAlignedPointsConstraint.isConstraintSolved","Not printing the Y values",true);
