 ...
 * mouse-press/release events. 'Track' adds the points to the stroke and thus
 * links the gui with the underlying plumbing.
 * 
 */

public class DrawingView extends JPanel implements MouseListener,
		MouseMotionListener, KeyListener, KeyEventDispatcher, Serializable

{

	private ProcessManager m_processManager;
	/**
	 * Strokes, constraints, markets, anchors, text etc added by
	 * recognizeSegmentsAndConstraints (primarily)
	 */
	...
	// threshold for distance between start and end points of a segment to be
	// deleted
	...
	/**
	 * This draws the component on-screen. AND repaints the entire screen - the
	 * grid,segments,text,markers etc. Is there any other way??
	 */
	public void paintComponent(Graphics gc)
	{
		...
		if (isM_gridActive() == true) {
			drGrid.drawGrid(gc, drViewHeight, drViewWidth,
					GVariables.GRID_COLOR);
		} else {
			drGrid.drawGrid(gc, drViewHeight, drViewWidth,
					GVariables.BACKGROUND_COLOR);
		}
		...
		/*
		 * if(winAct.getUndoBit() == 1){ drawMarkers(gc,
		 * m_drawData.getM_markers(),GVariables.BACKGROUND_COLOR);
		 * winAct.setUndoBit(0); } else{
		 */
	...
	public void addPointToStroke(Stroke strk , int x, int y, long time) 
	{
		...
		/*
		 * if (x > m_canvasUsed.width) m_canvasUsed.width = x; if (y >
		 * m_canvasUsed.height) m_canvasUsed.height = y;
		 */
	}

	/**
	 * Called by mouse-released. Detects Segment Points and does recognition and
	 * constraints
	 * 
	 * @param theStroke
	...
	public Vector addStroke(Stroke theStroke) {
		...
		PreProcessingManager preProcessMan = m_processManager
				.getPreProManager();
	...
	/**
	 * Calls a lot of methods in other parts of the program. This is the key
	 * method where everything is set in motion.
	 * 
	 * @param theStroke
	 * @return
	 */
	public Vector recognizeSegmentsAndConstraints(Stroke theStroke) {
		...
		StrokeRecognizer strokeRecog = recogMan.getM_strokeRecogManager()
				.getStrokeRecognizer();
		...
		while (iter.hasNext()) {
			...
			for (int i = 0; i < vecTemp.size(); i++) {
				...
				if (c instanceof IntersectionConstraint)
					c.remove();
				else if (c instanceof circularArcConstraint
		...
		if (stkType == Stroke.TYPE_MARKER) {
			...
			Vector constraints = theStroke.recognizeAllConstraints(m_drawData
					.getStrokeList());
			if (constraints != null)
	...
	public void recognizeSegments(Stroke theStroke) {
		...
		// identify segments
		try {
			SegmentRecognizer segmentRecog = recogMan.getSegmentRecogMan()
					.getSegmentRecognizer();
			// added on 23-02-10
			// if this stroke is converted, then no need to call segment
			// recognizer
			...
			JOptionPane.showMessageDialog(null,
					"Error Occured in recognize segments : " + e.getMessage());
			e.printStackTrace();
		}
	}
	 ...
	 * 
	 * Iterator iter = ips.iterator(); while (iter.hasNext()) { ImpPoint ip =
	 * (ImpPoint) iter.next(); snapIP(ip); } }
	 */
	// 08-02-10
	/**
	 * function snap the ips if a new stroke is drawn, check for if end points
	 * of this stroke needs to be snapped. In drawing mode, if a circular arc is
	 * drawn, then do not merge center of circle with any other point. Merge it
	 * only in Edit mode(when an element is being dragged)
	 * 
	 * @author Sunil Kumar
	 */
	public void snapIPs() {
		// found on 22-02-10 when we type a text then also the tool is in
		// Drawing mode.
		...
		if (!isEnterKeyClicked()) {
			if (GVariables.getDRAWING_MODE() == GConstants.DRAW_MODE
					&& m_drawData.getStrokeList().size() >= 1) {
				...
				if (m_currStroke != null) {
					if (m_currStroke.getM_segPtList().size() != 0) {
						...
						while (iter1.hasNext()) {
							...
							Vector anchorPoints = m_drawData
									.getAllAnchorPoints();
							...
							while (iter2.hasNext()) {
								...
								if (ip.getM_point() != null) {
									if ((ip.getM_point()).equals(segPoint)
											|| (ip.getM_point().distance(
													segPoint) < (((GConstants.cmScaleDrawingRatio) / 10) * 2))) {
				...
				if (isM_elementDragged() && (m_highlightedElements.size() > 0)) {
					System.out.println("highlighted elements count : "
							+ m_highlightedElements.size());
	...
	public boolean iterateParentVector(ImpPoint ip) {
		...
		while (iter.hasNext()) {
			Segment seg = (Segment) iter.next();
			// if this important point belongs to Circular arc and is center
			// point
			// don't merge
			if (seg instanceof SegCircleCurve) {
				SegCircleCurve segCC = (SegCircleCurve) seg;
	...
	public void mergePoints(ImpPoint ip1, ImpPoint ip2) {
		if (constraintsHelper.getNoMergeConstraintBetweenPoints(
				(AnchorPoint) ip1, (AnchorPoint) ip2) != null)
			return;
	...
	public void highlightTableRowsSelectedElems() {
		if (m_selectedElements != null) {
			...
			if (size != 0) {
				if (size == 1) {
					GeometryElement g1 = (GeometryElement) m_selectedElements
							.get(0);
					...
					GeometryElement g1 = (GeometryElement) m_selectedElements
							.get(0);
					GeometryElement g2 = (GeometryElement) m_selectedElements
							.get(1);
					...
					if (g1 instanceof AnchorPoint) {
						...
						if (g2 instanceof SegCircleCurve) {
							helpDrawView
									.selectRows(GConstants.SELECT_POINT_ARC);
						}
						if (g2 instanceof SegLine) {
							helpDrawView
									.selectRows(GConstants.SELECT_POINT_LINE);
						}
					}
						...
						if (g2 instanceof SegLine) {
							helpDrawView
									.selectRows(GConstants.MOVE_DELETE_ELEMENTS);
						}
					}
	...
	public void mousePressed(MouseEvent e) {
		...
		mousePressed(e.getButton(), e.getClickCount(), e.getX(), e.getY(), e
				.getWhen());
		// }
		// else{
			...
			if (m_highlightedElements.size() != 0) {
				if (m_keyEventCode == KeyEvent.VK_SHIFT) {
					if (m_highlightedElements.size() == 1) {
						GeometryElement ge = (GeometryElement) m_highlightedElements
								.get(0);
			...
			if (v.size() > 0) {
				...
				for (int i = 0; i < v.size(); i++) {
					if (v.get(i) instanceof pointOnLineConstraint) {
						...
						int index1 = l.getM_rawStartIdx(), index2 = l
								.getM_rawEndIdx();
						double d1 = l.getM_start().distance(p);
						double d = l.getM_length();
						int index3 = index1
								+ (int) (d1 * (index2 - index1) / d);
				...
				if (oldParentsVector.size() > 1) {
					...
					for (int i = 0; i < siz; i++) {
						...
						for (int j = i + 1; j < siz; j++) {
							...
							Vector tempVector = constraintsHelper
									.getRelativeConstraintsBetween2Segments(
											seg1, seg2);
					...
					while (oldParentsVector.size() != 0) {
						...
						AnchorPoint newAP = new AnchorPoint(new Point2D.Double(
								ap.getX(), ap.getY()), newParentsVector);
						newAPs.add(newAP);
						...
						if (seg instanceof SegLine) {
							...
							otherConstraints = (Vector) l.getM_start()
									.getConstraints().clone();
							if (l.getM_start() == ap)
								otherConstraints = (Vector) l.getM_end()
										.getConstraints().clone();
							resultingConstraints = constraintsHelper
									.minusInverse(ap.getConstraints(),
											otherConstraints);
						} else if (seg instanceof SegCircleCurve) {
							SegCircleCurve c = (SegCircleCurve) seg;
							AnchorPoint ap1 = c.getM_start(), ap2 = c
									.getM_end();
							if (ap == c.getM_start()) {
								ap1 = c.getM_end();
								ap2 = c.getM_center();
							...
							otherConstraints = constraintsHelper.minusInverse(
									ap1.getConstraints(), ap2.getConstraints());
							resultingConstraints = constraintsHelper
									.minusInverse(ap.getConstraints(),
											otherConstraints);
						} else
							System.out
									.println("Segment is an instance of unknown shape !!!");

						int size = resultingConstraints.size();
						for (int i = 0; i < size; i++) {
							Constraint c = (Constraint) resultingConstraints
									.get(i);
					...
					for (int i = 0; i < siz; i++) {
						...
						for (int j = i + 1; j < siz; j++) {
							...
							c = new NoMergeConstraint(ap1, ap2,
		...
		if (m_highlightedElements.size() > 0) {
			...
			if (m_highlightedElements.size() == 1) {
				System.out.println("Element Selected : "
						+ m_highlightedElements.get(0).getClass());
				String parsedCons[];

				parsedCons = m_highlightedElements.get(0).getClass().toString()
						.split("[ ]+");

				if (parsedCons[1]
						.compareToIgnoreCase("dcad.model.geometry.segment.SegLine") == 0) {
					System.out.println("Equal");
					Segment segm = (Segment) m_highlightedElements.get(0);

					if (segm instanceof SegLine)
						highlightedSegWhileDragging.add(segm);
					System.out.println("Added highlighted element : "
							+ (Segment) highlightedSegWhileDragging.get(0));
					System.out.println("size : "
							+ highlightedSegWhileDragging.size());
				}
			}
	...
	private void addConstraintsForMarkers() {
		...
		if ((m_drawData.isUnusedMarker()) || (m_drawData.isUnusedText())) {
			...
			// recognize markers
			RecognitionManager recogMan = ProcessManager.getInstance()
					.getRecogManager();
			MarkerRecogManager markerMan = recogMan.getMarkerRecognitionMan();
			MarkerToConstraintConverter converter = markerMan
					.getM_markerConverter();

			// check if there are new markers related to the text objects
			Vector newMarkers = converter.recognizeTextAsMarkers(m_drawData
					.getM_markers(), m_drawData.getM_textElements(), m_drawData
					.getAllSegments(), m_selectedElements,
					m_highlightedElements);
			...
			Vector constraints = converter.recognizeMarkersAsConstraints(
					m_drawData.getM_markers(), m_drawData.getM_textElements(),
					m_drawData.getAllSegments());
			...
			if (constraints != null && constraints.size() > 0) {
				...
				if (ConstraintSolver
						.addConstraintsAppliedUsingMarker(constraints) != null) {
					m_drawData.addConstraints(constraints);
				...
				// *************************************************************
					...
					updateConstraints(constraintsHelper
							.getListOfConstraints(m_drawData
									.getAllAnchorPoints()), Constraint.HARD);
	...
	// added on 19-04-10
	// checks whether the point is a child of how many elements
	/**
	...
	int ptOnSegments(Point pt)
	{
		...
		while (itr.hasNext()) {
			...
			while (iter.hasNext()) {
				...
				if ((Double.compare(pt.getX(), pt1.getX()) == 0)
						&& (Double.compare(pt1.getY(), pt.getY()) == 0)) {
					count++;
				}
			}
		}
	...
	// *******************************************************************
	/**
	 * Function to check whether Mouse_Button 3 is clicked on any element If yes
	 * then show appropriate parameter window
	 * 
	 * @author Sunil Kumar
	 */
	public void showElementPropertiesWindow(int x, int y, int buttonType) 
	{
		if ((m_currStroke == null || (m_currStroke.getLength() == 0))
				&& buttonType == MouseEvent.BUTTON1
				&& m_keyEventCode != KeyEvent.VK_SHIFT && this.hasFocus()) {
			Point pt = new Point();
			pt.setLocation(x, y);
			...
			if ((seg) != null && count < 2
					&& m_keyEventCode != KeyEvent.VK_CONTROL) {
	...
	public void mouseReleased(int x, int y, int buttonType) 
	{
		...
		// *******************************************
		// If the user is clicking in the blank area of the screen, do nothing.
		// Don't show the point
		// If user clicks twice at the same place, 2nd time, m_currStroke is set
		// to null. So, the condition checking it for null is required
		if (m_keyEventCode == -1 && m_currStroke != null
				&& m_currStroke.getLength() < Prefs.getAnchorPtSize())
			extraClick = true;

		if (isM_mousePressedLogged()) {
			logEvent("mouseReleased({int}" + x + ", {int}" + y + ", {int}"
					+ buttonType + ");");
		...
		else if (GVariables.getDRAWING_MODE() == GConstants.DRAW_MODE) 
		{
			...
			if (handleMouseDragEditMode(x, y)) {
				if (isM_elementDragged() && (m_highlightedElements.size() > 0)) {
					...
					// 06-10-09
					/*
					 * if(winAct == null){ winAct = WindowActions.getInstance();
					 * } if(winAct.getUndoVector() != null){
					 * if(winAct.getUndoIndex() <
					 * (winAct.getUndoVector().size()-1)){
	...
	/**
	 * Segmentation, constraints, drawing, everything.
	 * @param strk
	 * @return
	 */
	...
	/**
	 * Function to remove segment lines and their respective constraints if
	 * start point and end point of the segment are merged or the distance
	 * between two points is negligible
	 * 
	 * @author Sunil Kumar
	 */
	...
	public void sortAnchorPoints(double segPoints[][], int sortBy) {
		...
		for (int i = 0; i < n; i++) {
			System.out.println("Points  X: " + segPoints[i][0] + "Y: "
					+ segPoints[i][1]);
		}
	...
	public boolean isLineHorizontal(double angle1) {
		...
		if ((((Double.compare(angle1, 0.0) == 0) || (Double
				.compare(angle1, 0.0) > 0))
				&& (Double.compare(angle1, 0.5) == 0) || (Double.compare(
				angle1, 0.5) < 0))
				|| ((Double.compare(angle1, 179.5) == 0) || (Double.compare(
						angle1, 179.5) > 0))) {
			return true;
		}
		return false;
	...
	public boolean isLineVertical(double angle1) {
		...
		if (((Double.compare(angle1, 89.5) == 0) || (Double.compare(angle1,
				89.5) > 0))
				&& (Double.compare(angle1, 90.5) == 0)
				|| (Double.compare(angle1, 90.5) < 0)) {
			return true;
		}
		return false;
	...
	// added on 10-05-10
	// find the end point and starting point of other line according to
	// orientation
	/**
	...
	public double[][] findMiddlePtsWhileMoving(Segment seg1, Segment seg2) {
		...
		double[][] segPoints = new double[][] { { x1, y1 }, { x2, y2 },
				{ x3, y3 }, { x4, y4 } };
		...
		if (n > 0) {
			/*
			 * System.out.println("Sorted Points returned"); for(int i = 0; i <
			 * n; i++){ System.out.println("Points  X: " + segPoints[i][0] +
			 * "Y: " + segPoints[i][1]); }
			 */
	...
	/**
	 * function to to show a line connecting two lines, when they become
	 * collinear while dragging one of them
	 * 
	 * @author Sunil Kumar
	 */
	public void checkForCollinearLines() {
		...
		// to put highlighted element if it is a line in a segment
		if (m_highlightedElements.size() == 1
				&& m_button_type == MouseEvent.BUTTON3) {
			// System.out.println("Element Selected : " +
			// m_highlightedElements.get(0).getClass());
			String parsedCons[];
			...
			if (parsedCons[1]
				...
				// System.out.println("M_button_type :" + m_button_type +
				// "   Mouse Event : " + MouseEvent.BUTTON3);
				// System.out.println("size of highlighted elements : " +
				// m_highlightedElements.size());

				Segment seg = (Segment) m_highlightedElements.get(0);

				Vector parallelLinesConstraintList = getParallelLinesConsList(
						"lines", "parellel", seg);
				if (parallelLinesConstraintList.size() != 0) {
					// System.out.println("Size of elements :" +
					// parallelLinesConstraintList.size());
					int consNumber = 0;
					// for each parallel line to this find whether they are
					// collinear and display that line
					for (consNumber = 0; consNumber < parallelLinesConstraintList
							.size(); consNumber++) {
						Constraint c = (Constraint) parallelLinesConstraintList
								.get(consNumber);
						if (c instanceof RelativeConstraint) {
							...
							// seg1.getM_start().getX()+ ", Y1 "
							// +seg1.getM_start().getY());
							// System.out.println("X2 " +
							// seg1.getM_end().getX()+ ", Y2 "
							// +seg1.getM_end().getY());
							// System.out.println("X3 " +
							// seg2.getM_start().getX()+ ", Y3 "
							// +seg2.getM_start().getY());
							// System.out.println("X4 " +
							// seg2.getM_end().getX()+ ", Y4 "
							// +seg2.getM_end().getY());
							// check whether line formed by (x2,y2) and (x3,y3)
							// is parallel to any
							// other segment
							...
							/*
							 * int n= segPoints.length;
							 * System.out.println("Sorted Points"); for(int i =
							 * 0; i < n; i++){ System.out.println("Points  X: "
							 * + segPoints[i][0] + "Y: " + segPoints[i][1]); }
							 */
							if (constraintsHelper.areSlopesEqual(seg1
									.getM_start().getX(), seg1.getM_start()
									.getY(), seg1.getM_end().getX(), seg1
									.getM_end().getY(), segPoints[1][0],
									segPoints[1][1], segPoints[2][0],
									segPoints[2][1], false)) {
	...
	private boolean handleMouseDragEditMode(int x, int y) {
		...
		if (m_highlightedElements.size() > 0) {
			System.out.println("Mouse drag highlighted elements : "
					+ m_highlightedElements.size());
			...
			if (elementsToMove.size() > 0) {
				...
				// iter = elementsToMove.iterator();
				if (movedPts.size() > 0) {
					Vector affectedCons = ConstraintSolver
							.solveConstraintsAfterMovement(movedPts,
			...
			/*
			 * if(getMovedPointSegment() != null){ Segment movedSeg =
			 * getMovedPointSegment();
			 * 
			 * movedSeg.clearConstraints(Constraint.SOFT);
			 * movedSeg.clearConstraints(Constraint.HARD); //movedSeg.delete();
	...
	public double areaOfTriangle(AnchorPoint ptA, AnchorPoint ptB,
			AnchorPoint ptP) {
		return (Math.abs(ptA.getX() * ptB.getY() + ptB.getX() * ptP.getY()
				+ ptP.getX() * ptA.getY() - ptA.getX() * ptP.getY()
				- ptP.getX() * ptB.getY() - ptB.getX() * ptA.getY()) / 2);
	 ...
	 * @author Sunil Kumar
	 */
	public static Vector getParallelLinesConsList(String element,
			String constraint, Segment seg) {
		...
		while (itr.hasNext()) {
			...
			if ((parsedCons[4].compareToIgnoreCase(element) == 0)
					&& (parsedCons[3].compareToIgnoreCase(constraint) == 0)) {
	...
	public void mouseMoved(int x, int y) {
		...
		if (m_highlightedElements.size() == 0 && m_selectedElements.size() == 0) {
			if (m_drawData.isUnusedMarker()) {
				...
				if (mark instanceof MarkerAngle) {
					...
					if (seg instanceof SegCircleCurve) {
						helpDrawView
								.selectRows(GConstants.MARKER_PARALLEL_ON_ARC);
					} else if (seg instanceof SegLine) {
						helpDrawView
								.selectRows(GConstants.MARKER_PARALLEL_ON_LINE);
					}
		...
		/*
		 * Vector segPoints = new Vector(); segPoints =
		 * m_drawData.getAllAnchorPoints();
		 * 
		 * // Max distance within which the angle marker gets highlighted double
		 * maxDistance = GConstants.cmScaleDrawingRatio/2; AnchorPoint point =
		 * new AnchorPoint();
		 * 
		 * // To find the anchor point closest to the mouse pointer Iterator
		 * iter = segPoints.iterator(); while(iter.hasNext()){ AnchorPoint ap =
		 * (AnchorPoint)iter.next(); if(ap.distance(m_mousePos) <= maxDistance){
		 * point = ap; // System.out.println("close to point " + point.getX() +
		 * point.getY()); break; } }
		 * 
		 * // Vectors containing segment list and line segments having this
		 * point Vector segList = new Vector(); Vector lineSegsContainAP = new
		 * Vector(); segList = m_drawData.getAllSegments();
		 * 
		 * // get the list of segments containing this anchor point iter =
		 * segList.iterator(); while(iter.hasNext()){ Segment seg =
		 * (Segment)iter.next(); if(seg instanceof SegLine){
		 * if((seg.getSegStart() == point) || (seg.getSegEnd() == point) ){
		 * lineSegsContainAP.add((SegLine)seg); } } }
		 * 
		 * //System.out.println("size is" + lineSegsContainAP.size()); // find
		 * the point is near to which lines int vectorSize =
		 * lineSegsContainAP.size(); // if there are only two lines whether this
		 * point lies outside the // triangle formed by these points Vector
		 * pointsOfTriangle = new Vector(); if( vectorSize == 2){ iter =
		 * lineSegsContainAP.iterator(); while(iter.hasNext()){ Segment seg =
		 * (Segment)iter.next(); if(seg.getSegStart() != point){
		 * pointsOfTriangle.add((AnchorPoint)seg.getSegStart()); } else{
		 * pointsOfTriangle.add((AnchorPoint)seg.getSegEnd()); } } AnchorPoint
		 * ptA = (AnchorPoint)pointsOfTriangle.get(0); AnchorPoint ptB =
		 * (AnchorPoint)pointsOfTriangle.get(1); AnchorPoint ptC = point; Point
		 * ptP = m_mousePos;
		 * 
		 ...
		 * 
		 * double errorMargin = 0.000000000000001; SegLine lineAB, lineAC;
		 * if(areaABC == (areaABP + areaBCP + areaACP)){
		 * 
		 * // angle of line AB with X-axis lineAB =
		 * (SegLine)lineSegsContainAP.get(0); double angleAB =
		 * findAngleWithXAxis(lineAB.getSegStart(),lineAB.getSegEnd());
		 * 
		 * 
		 * //angle of line AC with X-axis lineAC =
		 * (SegLine)lineSegsContainAP.get(1); double angleAC =
		 * findAngleWithXAxis(lineAC.getSegStart(),lineAC.getSegEnd());
		 * 
		 * 
		 * // System.out.println("Point is inside the triangle with slope " +
		 * angleAB); //
		 * System.out.println("Point is inside the triangle with slope " +
		 * angleAC);
		 * 
		 * // draw a circular arc between these two lines Marker m_marker =
		 * null; m_marker = new MarkerAngle(null, lineAB, lineAC, null); int
		 * markerType = m_marker.getM_type(); } else{ //
		 ...
		 * 
		 * 
		 * lineSegsContainAP.clear(); /* if ((ele.isSelected()) &&
		 * (m_selectedElements.contains(ele))) { ele.setSelected(false);
		...
		if (!isParameterWinBitSet()) {
			...
			if (m_highlightedElements.size() > 0) {
				...
				if (!repaintReq)
					// mouse is still on the same object.. no need to do
					// anything.
					return;
				else {
					// addToUndoVector();
			...
			if ((m_keyEventCode != -1) && (m_keyEventCode == KeyEvent.VK_SHIFT)) {
				if (m_showLastStroke) {
					Stroke lastStroke = m_drawData.getLastStroke(true);
					if ((lastStroke != null)
							&& (lastStroke.containsPt(m_mousePos)))
				...
				if (m_keyEventCode == KeyEvent.VK_CONTROL) {
					for (int i = 0; i < gEles.size(); i++) {
						if (gEles.get(i) instanceof SegLine) {
							...
							for (int j = 0; j < equalRelativeLengthConstraints
									.size(); j++) {
			...
			if (m_highlightedElements.size() > 0) {
				// 04-10-09 to highlight rows in HELP table
				GeometryElement g1 = (GeometryElement) m_highlightedElements
						.get(0);
				if (m_selectedElements.size() == 0) {
					if ((g1 instanceof SegLine)
							|| (g1 instanceof SegCircleCurve)) {
						if (m_keyEventCode == KeyEvent.VK_CONTROL) {
							helpDrawView.selectRows(GConstants.SELECT_ELEMENT);
						} else if (m_keyEventCode == KeyEvent.VK_SHIFT) {
							helpDrawView.unselectRows();
							...
							helpDrawView
									.selectRows(GConstants.HIGHLIGHT_ELEMENTS);
						}
						...
						if (m_keyEventCode == KeyEvent.VK_SHIFT) {
							helpDrawView
									.selectRows(GConstants.REMOVE_ANCHOR_POINT); // unselect
																					// row
																					// 7
			...
			/*
			 * else{ helpDrawView.unselectRows(); }
			 */
			updateStatusBar(x, y, "( Move )", label);
		} else {
	...
	private Vector performSegFusion(AnchorPoint ap) {
		...
		// 03-05-10
		// condition is added to avoid array index out of bounds exception in
		// case parents size is null
	...
	private Vector performReSegmentation(Stroke theStroke, Point pt) {
		...
		if ((ppi != null) && (npi != null)) {
			...
			PixelInfo newPI = new PixelInfo(pt, (long) ((ppi.getTime() + npi
					.getTime()) / 2));
			...
			theStroke.getM_ptList().insertElementAt(newPI,
					theStroke.getM_ptList().indexOf(npi));
			...
			JOptionPane.showMessageDialog(null, "No Close Pixel found ",
					"Resegmentation Error", JOptionPane.ERROR_MESSAGE, null);
			return null;
		}
	}
	...
	public void adjustStroke(Stroke theStroke) {
		...
		if (theStroke.getM_segList().size() >= 2) {
			...
			while (iter.hasNext()) {
				...
				prevSeg.getSegEnd().move(nextSeg.getSegStart().getX(),
						nextSeg.getSegStart().getY());
				// 18-2-2008
	...
	public void keyTyped(KeyEvent e) {
		...
		if ((((c >= '0') && (c <= '9')) || ((c >= 'a') && (c <= 'z'))
			...
			// getGraphics().drawString(c + "", m_mousePos.x + width,
			// m_mousePos.y);
		} else {
			switch (c) {
			...
			case KeyEvent.VK_ENTER:
				...
				if (typedText.length() > 0) {
					...
					for (int i = 0; i < typedText.length(); i++)
						blankStr += " ";
					getGraphics().drawString(blankStr, m_mousePos.x,
							m_mousePos.y);
	...
	private void backspacePressed() {
		...
		if (typedText.length() > 0) {
			...
			g2D.fillRect(m_mousePos.x + fm.stringWidth(typedText), m_mousePos.y
					- height, fm.stringWidth(remain), height);
		...
		if (gEle != null) {
			...
			if (gEle instanceof ImpPoint) {
				for (int l = 0; l < ((ImpPoint) gEle).getAllParents().size(); l++) {
					movedElement = (Segment) ((ImpPoint) gEle).getAllParents()
							.elementAt(l);
	...
	public void snapIPsAndRecalculateConstraints() {
		...
		Vector newlyAddedConstraints = recalculateConstraints(m_drawData
				.getAllSegments());

		// These constraints could be solved
		Vector solvedConstraints = ConstraintSolver
				.solveConstraintsAfterSnapAndRecalculation(m_drawData
						.getAllAnchorPoints());
		...
		updateConstraints(constraintsHelper.getListOfConstraints(m_drawData
				.getAllAnchorPoints()), Constraint.HARD);
		// commented on 15-10-09
		// Because due to it, if user draws some small stroke it gets removed
		// which th user does not want
		/* 8-5-2008 */
		// Don't forget to clone it
		/*
		 * Vector allSegments = (Vector) m_drawData.getAllSegments().clone();
		 * Segment seg; for (int i = 0; i < allSegments.size(); i++) { seg =
		 * (Segment) allSegments.get(i); if (seg instanceof SegLine &&
		 * ((SegLine) seg).getM_length() < Constraint.MAX_ALLOWED_CONNECT_GAP)
		 * {}//seg.delete(); else if (seg instanceof SegCircleCurve &&
		 * ((SegCircleCurve) seg).getM_radius() <
		 * Constraint.MAX_ALLOWED_CONNECT_GAP) {}//seg.delete(); }
		 */
	...
	/**
	 * Called by only SnapIPsandrecalcConstraints...
	 * 
	 * @param affectedSegs
	 * @return
	 */
	...
	/**
	 * Clear constraints(SOFT) of the segment and find them again.
	 * 
	 * @param gElement
	 * @return
	 */
	public Vector recalculateConstraints(GeometryElement gElement) {
		...
		if (gElement != null) {
			...
			for (int i = 0; i < allMovedElements.size(); i++) {
				// there will be only one moved_element [unless IMPpoint]

				movedElement = (GeometryElement) allMovedElements.elementAt(i);
	...
	private void resizePanel() {
		...
		/*
		 * if (stk==null) return; Vector vec = stk.getM_ptList(); for(int
		 * i=0;i<vec.size();i++) { Point p = ((PixelInfo) vec.get((Integer)i));
		 * //AnchorPoint p = (AnchorPoint)v.get(i); if(p.getX() >
		 * m_canvasUsed.width) m_canvasUsed.width = (int)p.getX() + 100;
		 * if(p.getY() > m_canvasUsed.height) m_canvasUsed.height =
		 * (int)p.getY() + 100; }
		 */
		...
		for (int i = 0; i < v.size(); i++) {
			...
			if (p.getX() > m_canvasUsed.width)
				m_canvasUsed.width = (int) p.getX()
						+ ((int) GConstants.cmScaleDrawingRatio * 3);
			if (p.getY() > m_canvasUsed.height)
				m_canvasUsed.height = (int) p.getY()
						+ ((int) GConstants.cmScaleDrawingRatio * 3);
		}
	...
	/**
	 * Update the recognized view constraint list and the edit pane. TODO: Fix
	 * point null problem.
	 * 
	 * @param type
	 *            1=only constraints 2=everything, including edit pane
	 * @param cons
	 */
	public void UpdateUI(int type, Vector cons) {
		...
		// Need mouse-pointer location to get the segment, if the segment/stroke
		// is null. Mouse-press creates new stroke, over writing previous one,
		// so this
		// is needed.
		...
		if (segm != null) {
			...
			System.out
					.println("BEGIN POINT CO_ORDINATES......................");
